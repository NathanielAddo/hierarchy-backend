...
const initializeDatabase = async () => {
    try {
        ...
        let users = [];
        try {
            const eventsResponse = await axios_1.default.get("https://db-api-v2.akwaabasoftware.com/attendance/meeting-event", {
                headers: { Authorization: `Token ${adminToken}` },
            });
            ...
            const schedulesResponse = await axios_1.default.get("https://db-api-v2.akwaabasoftware.com/attendance/meeting-event/schedule", {
                headers: { Authorization: `Token ${adminToken}` },
                params: {
                    isRecuring: "both",
                    length: 100000,
            ...
            if (axios_1.default.isAxiosError(error)) {
                console.warn("Failed to fetch schedules, skipping schedule migration:", error.response?.data || error.message);
            }
            else {
                console.warn("Failed to fetch schedules, skipping schedule migration:", error.message || "Unknown error");
            }
        }
        ...
        for (const admin of admins) {
            let mainAccount = await accountRepository.findOne({
                where: { id: admin.accountId ? (0, uuid_1.v4)() : admin.accountId },
            });
            if (!mainAccount) {
